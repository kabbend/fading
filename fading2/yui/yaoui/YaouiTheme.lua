...
YaouiTheme.colors = {
    ...
    checkbox_v = {255, 255, 255},

    dropdown_primary = {40, 15, 15},
    dropdown_area_bg = {32, 32, 32},
    dropdown_button_selected_bg = {20, 32, 48},

    --flat_dropdown_bg = {238, 231, 231},
    flat_dropdown_bg = {255, 255, 255},
...
YaouiTheme.Button.draw = function(self)
    ...
    --if self.fuzz then self.color = {20,20,20} end
...
YaouiTheme.Checkbox.draw = function(self)
    ...
    elseif self.checked_exit then self.timer:tween('check', 0.05, self, {check_alpha = 0}, 'linear', function() self.check_draw = false end) end
...
YaouiTheme.FlatDropdown.draw = function(self)
    ...
    --love.graphics.setColor(unpack(self.yui.Theme.colors.text))
    love.graphics.setColor(40,15,15)
    local font = love.graphics.getFont()
    love.graphics.setFont(self.font)
    love.graphics.print(self.parent.options[self.parent.current_option] .. ' ', 
...
YaouiTheme.Text.draw = function(self)
    ...
    love.graphics.setFont(self.font)
    if (self.center) then
      love.graphics.printf(self.text, self.x , self.y + self.font:getHeight()/2 - self.size / 4, self.w, 'center')
      else
      love.graphics.print(self.text, self.x + self.size/6, self.y + self.font:getHeight()/2 - self.size / 4)
    end
    if (self.text2 and self.font2) then
      love.graphics.setFont(self.font2)
      love.graphics.printf(self.text2, self.x + self.size * 1.4, self.y + self.font2:getHeight()/2 - self.size / 4, self.w, 'center')
    end   
    if (self.text3 and self.font3) then
      love.graphics.setFont(self.font3)
      love.graphics.printf(self.text3, self.x + self.size * 1.4, self.y + self.font2:getHeight()/2 + self.size / 4, self.w, 'center')
    end 
    love.graphics.setFont(font)
    love.graphics.setColor(255, 255, 255)
end
