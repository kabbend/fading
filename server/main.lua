...
function init() 
    ...
    io.write("main.lua: scenario directory : " .. scenarioDirectory .. "\n") ; layout.notificationWindow:addMessage("scenario : " .. scenarioDirectory .. "\n")
    ...
    if __WINDOWS__ then
      ...
      scenarioDirectoryCp1252 =   codepage.utf8tocp1252( scenarioDirectory )
      io.write("main.lua: cp1252 scenario directory : " .. scenarioDirectoryCp1252 .. "\n")

      local uWindow = urlWindow:new{w=1000,h=38,x=1000-layout.W/2,layout=layout, path=baseDirectoryCp1252..sep..scenarioDirectoryCp1252..sep,
					y= -layout.H/2+theme.iconSize+layout.intW+2*layout.snapshotSize }
      layout:addWindow( uWindow , false, "uWindow" )

    else

      local uWindow = urlWindow:new{w=1000,h=38,x=1000-layout.W/2,layout=layout, path=baseDirectory..sep..scenarioDirectory..sep,
					y= -layout.H/2+theme.iconSize+layout.intW+2*layout.snapshotSize }
      layout:addWindow( uWindow , false, "uWindow" )
    ...
    local directory, scenarioDirectory = baseDirectory, scenarioDirectory
    if __WINDOWS__ then directory, scenarioDirectory = baseDirectoryCp1252, scenarioDirectoryCp1252 end
    ...
    if scenarioDirectory and scenarioDirectory ~= "" then
      parseDirectory{ path = baseDirectory .. sep .. scenarioDirectory }
      parseDirectory{ path = baseDirectory .. sep .. scenarioDirectory .. sep .. "pawns" , kind = "pawns" }
      parseDirectory{ path = baseDirectory .. sep .. scenarioDirectory .. sep .. "maps" , kind = "maps" }
    end
