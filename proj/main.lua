...
ready = 		false				-- are we ready to receive commands ?
serverOS = 		nil				-- OS X, Windows etc. This tells us if we need to redress filenames	
tcp = 			nil				-- tcp connection to the server, once ip is determined	
timer 			= 0
connectRetryTime 	= 6
...
-- use stencils to draw fog of war
-- stencils are managed differently between version 0.9.x and 0.10.x
...
function love.update( dt )
	...
	if not connect and address and timer > connectRetryTime then
		io.write("calling server...\n")
 		...
 		tcp = socket.tcp()
 		tcp:settimeout(3)
 		local succ, msg = tcp:connect(address, port) 
		if succ then
			connect = true
	...
	if connect then data, msg = tcp:receive() end
...
function love.load( args )
 ...
 io.write("love version " .. major .. "." .. minor .. "." .. revision .. " " .. codename .. "\n")
 assert(major==0 and (minor==9 or minor==10),"Sorry. Only runs with Love2d 0.9.x or 0.10.x")

 dofile("proj/pconf.lua")

 -- PRIMARY SERVER
 address = nil		-- we don't know the server IP. we will scan for it
  
 -- but we know the ports
 scan = scanport or 12347
 port = serverport or 12345
 portbin = port + 1
